...
func initialize_game():
	...
	# Try to load the first generated level if it exists, otherwise fall back to default grid
...
func load_level_from_json(path: String) -> bool:
	...
	# Apply level data to grid_manager
	...
	# Reset grid visual representation
	...
	print("Successfully loaded level: ", path, " (Par: ", level_data.get("min_moves", -1), ")")
	return true
...
func generate_and_save_50_levels():
	...
	while count < 50 and attempts < max_attempts:
		attempts += 1
		# Width = 7, Height = 7, Walls = 7, Gems = 3
		...
		if moves > 0:
			# Solvable! AddPar value and save
	...
	if debug_status_label:
		if count == 50:
			...
			# Reload level 1 to play it immediately
